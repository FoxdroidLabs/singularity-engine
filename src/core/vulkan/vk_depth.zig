...
pub const VulkanDepth = struct {
    ...
    pub fn init(logDevice: *const vk.DeviceProxy, extent: vk.Extent2D, instance: vk.InstanceProxy, device: vk.PhysicalDevice) !VulkanDepth {
        ...
        const depth_memory = try logDevice.allocateMemory(&.{ .allocation_size = depth_mem_req.size, .memory_type_index = try findMemoryType(instance, device, depth_mem_req.memory_type_bits, .{ .device_local_bit = true }) }, null);
        try logDevice.bindImageMemory(depth_image, depth_memory, 0);

        const depth_view = try logDevice.createImageView(&.{ .image = depth_image, .view_type = .@"2d", .format = .d32_sfloat, .components = .{ .r = .identity, .g = .identity, .b = .identity, .a = .identity }, .subresource_range = .{
        ...
        return .{ .depth_view = depth_view, .depth_image = depth_image, .depth_memory = depth_memory };
    }
