...
pub const Camera = struct {
    ...
    pub fn update(self: *Camera, window: *glfw.Window, dt: f32) void {
        ...
        if (right_pressed and !self.rotating) {
            self.rotating = true;
        ...
        const right = math.Vector.normalize(
            math.Vector.cross(f, .{ .x = 0.0, .y = 1.0, .z = 0.0 }),
        );

        const spd = self.speed * dt;

        if (window.getKey(.w) == .press)
            self.pos = math.Vector.add(self.pos, math.Vector.scale(f, spd));

        if (window.getKey(.s) == .press)
            self.pos = math.Vector.sub(self.pos, math.Vector.scale(f, spd));

        if (window.getKey(.a) == .press)
            self.pos = math.Vector.sub(self.pos, math.Vector.scale(right, spd));

        if (window.getKey(.d) == .press)
            self.pos = math.Vector.add(self.pos, math.Vector.scale(right, spd));
